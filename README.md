...
Once inside the Docker container, you can follow the instructions in the `RL-Oracle-Lyapunov-v1/Readme.md` under the **Usage** section.
...
1. Choose and inject the bug:
    ...
    The algorithms and bugs you choose must correspond to each other; if you need more information, please check the bug document in **\RLTestingLyapunov\Bug library** file. Specially, if bug_no = -1, it means inject no bugs into SB3 library.
    ```python
    bug_no = -1
    algorithm = 'ppo'
    n = 2
...
2. Inject bug into Stable-baselines3 library
    ...
    **Important**: Restart the kernel after injecting the bug to make sure the running stable-baselines3 is injected successfully. After the kernel restarts, you don't have to run this block again.
...
5. Train **I** agents and save all the trained agents into a file for further usage  

    This step may take some time, depending on your device and the algorithm you choose. Normally, a2c and ppo can run quickly but td3 is slow. If I = 20, a2c and ppo need 45 min, td3 needs 2 hours to run.
